...
@Component({
  selector: 'app-service-order-manual',
  templateUrl: './service-order-manual.component.html',
  styleUrls: ['./service-order-manual.component.css']
})
export class ServiceOrderManualComponent implements OnInit {

  selection = new SelectionModel<ServiceOrderManualComponent>(true, []);
   ...
   //selection: any;
    ...
    constructor(private http: HttpClient, private cookieService: CookieService, private fb: FormBuilder,
      private dialog: MatDialog, private router: Router, private snackBar: MatSnackBar , private cd: ChangeDetectorRef) {
        this.username = this.cookieService.get('manually');
    ...
    checkboxLabel(row?: ServiceOrderManualComponent): string {
      if (!row) {
        ...
        if (result === 'confirm') {
          ...
          this.http.post('/api/invoices/' + invoice.username.toUpperCase(), {
            ...
            setTimeout(function(){
              ...
              document.cookie = "manually=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
            },22000);
          }, err => {
            console.log(err);
          });
